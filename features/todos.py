...
async def handle_todo_intent(
    ...
    if intent == TODO_ADD:
        ...
        if not task_text:
            await ctx.reply('What should I add? Try: "add todo finish the report"')
            return
        ...
        if result:
            ...
            await ctx.reply_markdown(f"✓ Added: *{task_text}*{suffix}")
        else:
            await ctx.reply("Sorry, I couldn't add that. Try again.")
        return
    ...
    if intent == TODO_LIST:
        from adapters.google_tasks import list_todos
        todos = list_todos(svc, include_done=False)
        await ctx.reply_markdown(_format_todo_list(todos))
        return
    ...
    if intent == TODO_COMPLETE:
        ...
        if complete_todo(svc, match["id"]):
            await ctx.reply_markdown(f"✓ Marked complete: *{match['title']}*")
        else:
    ...
    if intent == TODO_DELETE:
        ...
        if not query:
            await ctx.reply('Which todo should I delete? Try: "delete todo [name]"')
            return

        match = _find_best_match(todos, query)
        if not match:
            await ctx.reply(f'No todo found matching "{query}". Run /todos to see your list.')
            return
    ...
    if intent == TODO_UPDATE:
        ...
        if not query or not new_text:
            await ctx.reply('To update a todo, say: "change [old name] to [new name]"')
            return
        ...
        if result:
            await ctx.reply_markdown(f"✓ Updated: *{new_text}*")
        else:
