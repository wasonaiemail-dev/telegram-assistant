...
async def handle_gift_intent(
    ...
    if intent == GIFT_ADD:
        ...
        if not recipient:
            await ctx.reply("Who is this gift for? Try: \"add gift idea for [name]: [idea]\"")
            return

        if not idea:
            await ctx.reply(f"What's the gift idea for {recipient}? "
        ...
        if result:
            extras = [x for x in [occasion, date] if x]
            suffix = f" _({', '.join(extras)})_" if extras else ""
            await ctx.reply_markdown(f"🎁 Added: *{recipient}* — {idea}{suffix}")
        else:
            await ctx.reply("Couldn't save that gift idea. Try again.")
        return
    ...
    if intent == GIFT_DONE:
        ...
        if not recipient and not idea:
            await ctx.reply("Which gift did you purchase? Try: \"bought [idea] for [name]\"")
            return
        ...
        if not match:
            ...
            await ctx.reply(f"I couldn't find a gift idea matching \"{query}\". Run /gifts to see your list.")
            return

        if complete_gift(svc, match["id"]):
            await ctx.reply_markdown(f"🎁 Marked as purchased: *{match['_person']}* — {match['_idea']}")
        else:
            await ctx.reply("Couldn't mark that as purchased. Try again.")
        return
    ...
    if intent == GIFT_DELETE:
        ...
        if not recipient and not idea:
            await ctx.reply("Which gift idea should I remove? Try: \"remove gift idea for [name]\"")
            return
        ...
        if delete_gift(svc, match["id"]):
            await ctx.reply_markdown(f"✓ Removed: *{match['_person']}* — {match['_idea']}")
        else:
