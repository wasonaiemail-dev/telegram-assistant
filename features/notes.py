...
async def handle_note_intent(
    ...
    if intent == NOTE_ADD:
        ...
        if not text:
            await ctx.reply('What should I save? Try: "note: remember to call the dentist"')
            return
    ...
    if intent == NOTE_LIST:
        ...
        notes = list_notes(svc)
        await ctx.reply_markdown(_format_notes_list(notes))
        return
    ...
    if intent == NOTE_DELETE:
        ...
        if not notes:
            await ctx.reply("No notes to delete.")
            return
        ...
        if number is not None:
            try:
                ...
                idx = -1
            if not (0 <= idx < len(notes)):
                await ctx.reply(f"Note #{number} doesn't exist. Run /notes to see your list.")
                return
            note = notes[idx]
        elif text:
            note = _find_note_by_text(notes, text)
            if not note:
                await ctx.reply(f'Couldn\'t find a note matching "{text}". Run /notes to see your list.')
                return
        else:
            await ctx.reply('Which note should I delete? Try: "delete note 2" or "delete note about [topic]"')
            return
    ...
    if intent == NOTE_EDIT:
        ...
        if not new_text:
            await ctx.reply('What should the note say? Try: "edit note 2 to say [new text]"')
            return
        from adapters.google_tasks import list_notes, update_note
        notes = list_notes(svc)
        if not notes:
            await ctx.reply("No notes to edit.")
            return
        note = _resolve_note_ref(ref, notes)
        if not note:
            await ctx.reply("Couldn't find that note. Run /notes to see your list.")
            return
    ...
    if intent == NOTE_APPEND:
        ...
        if not append_text:
            await ctx.reply('What should I add? Try: "append to note 2: also bring passport"')
            return
        from adapters.google_tasks import list_notes, update_note
        notes = list_notes(svc)
        if not notes:
            await ctx.reply("No notes to append to.")
            return
        note = _resolve_note_ref(ref, notes)
        if not note:
            await ctx.reply("Couldn't find that note. Run /notes to see your list.")
