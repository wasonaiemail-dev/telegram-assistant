...
| Long-term memory empty right after the Marvin→Marvin rename | One-time auto-migration in `load_memory()` renames legacy `marvin_memory.json` → `marvin_memory.json` on first access. Runs only if the new file is missing and the old one exists; safe no-op otherwise. |
